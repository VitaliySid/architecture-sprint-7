...
| 1   | Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам?                                       |   Нет    |      Технически можно настроить через keycloak        |
...
| 4   | Реализован ли принцип минимальных привилегий при назначении ролей и прав доступа?                                                        |   Нет    |       Нет данных       |
...
| 9   | Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?                                                        |   Да    |      Бухгалтерия в отдельном контуре, но при этом данные по взаиморасчетам с клиентами не в отельном контуре         |
| 10  | Ограничен ли доступ к системам из внешних источников?                                                                                    |   Да    |       Есть firewall       |
| 11  | Ведётся ли единый реестр учётных записей?                                                                                               |   Нет    |       Покупатели аутентифицируются через keycloak, внутренние сотрудники через один AD, бухгалтерия через другой AD       |
...
| 1   | Разработана ли политика классификации данных, и актуализируется ли она в соответствии с действующим законодательством?                  |   Нет    |      Судя по тому, что все данные идут в один DWH        |
...
| 8   | Осуществляется ли мониторинг доступа к данным на основе их классификации? Используются ли метки безопасности для маркировки данных?     |    Нет   |       Некоторые контракты позволяют внешним пользователям выполнять действия и получать данные, к которым у них не должно быть доступа       |
